...
public abstract class BaseJdbcDaoImpl extends BaseLogger implements IBaseJdbcDao {
	...
	@Override
	public <T> List<T> findListDataByFinder(Finder finder, Page page, Object queryBean)
			throws Exception {
		...
		if (page == null) {
			...
			return (List<T>) this.queryForList(finder, page);
		}

		// 根据page的参数 添加 order by
		getFinderOrderBy(finder, page);

		List<T> datas = (List<T>) this.queryForList(finder, page);
		if (datas == null) {
			datas = new ArrayList<T>();
		}
		return datas;
