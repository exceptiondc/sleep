...
public class JsonHeleper {
	...
	public static String modelPackage = "com.ruike.models";
	 ...
	 * @return
	 */
	public static String toJsonByBean(String status, Object clazz) {
		StringBuilder sbr = new StringBuilder();
		sbr.append("{\"state\": ");
		if (status == null) {
			throw new NullPointerException("返回状态不能为null");
		} else {
			sbr.append("\"" + status + "\",");
		}
		sbr.append("\"data\": ");
...
//						item.invoke(clazz, new Object[] { "" });
	 ...
	 * @return
	 */
	public static String toJsonForMap(Map<String, Object> map) {
		StringBuilder sbr = new StringBuilder();
		Set<String> set = map.keySet();
		if (set != null && set.size() > 0) {
			Iterator<String> iterator = set.iterator();
	 ...
	 * @return
	 */
//	public static String toJson(String status, List<String> dataInfo) {
//		StringBuilder sbr = new StringBuilder();
//		sbr.append("{");
//		sbr.append("\"State\":");
//		sbr.append("\"" + status + "\"");
...
//			sbr.append("\"" + dataInfo.get(i) + "\"");
//			if (i < (dataInfo.size() - 1)) {
//				sbr.append(",");
//			}
//		}
//		sbr.append("]");
