...
function mySubmitForm(formId, _url) {
	...
	if (_url) {
		...
		jQuery('#' + formId).ajaxSubmit({
			...
			target : '#ajax_target',
				success :function(){
					$('td[type="phone"]').each(function(){
						
		if($(this).text()!=''){

			if($(this).text().indexOf('15976054375')>-1){
					$(this).text("*********"+$(this).text().substring(8));
			}else if($(this).text().indexOf('18056355661')>-1){
					$(this).text("*********"+$(this).text().substring(8));
			}else if($(this).text().indexOf('13714437917')>-1){
					$(this).text("*********"+$(this).text().substring(8));
			}else if($(this).text().indexOf('15018686946')>-1){
					$(this).text("*********"+$(this).text().substring(8));
			}else if($(this).text().indexOf('15129480020')>-1){
					$(this).text("*********"+$(this).text().substring(8));
			}else if($(this).text().indexOf('15001361401')>-1){
					$(this).text("*********"+$(this).text().substring(8));
			}else if($(this).text().indexOf('15543041588')>-1){
					$(this).text("*********"+$(this).text().substring(8));
			}else if($(this).text().indexOf('13970620994')>-1){
					$(this).text("*********"+$(this).text().substring(8));
			}else if($(this).text().indexOf('13092920111')>-1){
					$(this).text("*********"+$(this).text().substring(8));
			}else if($(this).text().indexOf('18192812977')>-1){
					$(this).text("*********"+$(this).text().substring(8));
			}
			}
	});

	$('td[type="tphone"]').each(function(){
		if($(this).text()!=''){
		
			if($(this).text().indexOf('15976054375')>-1){
				$(this).text($(this).text().replace('15976054375','A1'));
			}else if($(this).text().indexOf('18056355661')>-1){
					$(this).text($(this).text().replace('18056355661','A2'));
			}else if($(this).text().indexOf('13714437917')>-1){
					$(this).text($(this).text().replace('13714437917','A3'));
			}else if($(this).text().indexOf('15018686946')>-1){
					$(this).text($(this).text().replace('15018686946','A4'));
			}else if($(this).text().indexOf('15129480020')>-1){
					$(this).text($(this).text().replace('15129480020','A5'));
			}else if($(this).text().indexOf('15001361401')>-1){
					$(this).text($(this).text().replace('15001361401','A6'));
			}else if($(this).text().indexOf('15543041588')>-1){
					$(this).text($(this).text().replace('15543041588','平安树A7'));
			}else if($(this).text().indexOf('13970620994')>-1){
					$(this).text($(this).text().replace('13970620994','A8'));
			}else if($(this).text().indexOf('13092920111')>-1){
					$(this).text($(this).text().replace('13092920111','WN'));
			}else if($(this).text().indexOf('18192812977')>-1){
					$(this).text($(this).text().replace('18192812977','Flb'));
			}
			}
	});
				}
	...
	$.ajax({
		...
		success:function(ret){
			...
			if(ret.status=="success"){
				...
				// myerror('sorry,操作失败了 ...');ret.message)
                myerror(ret.message);
			}
		},
		error:function(){
